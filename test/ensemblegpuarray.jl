...
const pre_p = [rand(Float32, 3) for i = 1:10]
...
@test all(all(sol[i].prob.p .== pre_p[i] .* p) for i = 1:10)
@test all(all(sol2[i].prob.p .== pre_p[i] .* p) for i = 1:10)
...
# test continuous
continuous_callback =
    ContinuousCallback(c_condition, c_affect!, save_positions = (false, false))
...
function rober_f(internal_var___du, internal_var___u, internal_var___p, t)
    @inbounds begin
        internal_var___du[1] =
            ...
            internal_var___p[3] * internal_var___u[2] * internal_var___u[3]
        internal_var___du[2] =
            (
                internal_var___p[1] * internal_var___u[1] -
...
function rober_jac(internal_var___J, internal_var___u, internal_var___p, t)
    @inbounds begin
        ...
        internal_var___J[2, 2] =
            -2 * internal_var___p[2] * internal_var___u[2] -

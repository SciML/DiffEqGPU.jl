...
sol = solve(monteprob, GPUVern7(), EnsembleGPUKernel(), trajectories = 10,
            adaptive = false, dt = 0.01f0)
asol = solve(monteprob, GPUVern7(), EnsembleGPUKernel(), trajectories = 10,
...
bench_sol = solve(prob, Vern7(), adaptive = false, dt = 0.01f0)
bench_asol = solve(prob, Vern7(), dt = 0.1f-1, save_everystep = false, abstol = 1e-8,
                   reltol = 1e-5)
...
sol = solve(monteprob, GPUVern7(), EnsembleGPUKernel(), trajectories = 2,
            adaptive = false, dt = 0.01f0, saveat = saveat)

asol = solve(monteprob, GPUVern7(), EnsembleGPUKernel(), trajectories = 2,
             adaptive = true, dt = 0.1f-1, abstol = 1.0f-8, reltol = 1.0f-5,
             saveat = saveat)

bench_sol = solve(prob, Vern7(), adaptive = false, dt = 0.01f0, saveat = saveat)
bench_asol = solve(prob, Vern7(), dt = 0.1f-1, save_everystep = false, abstol = 1e-8,
...
@test norm(bench_asol.u - asol[1].u) < 1e-3
...
bench_sol = solve(prob, Vern7(), adaptive = false, dt = 0.01f0, save_everystep = false)

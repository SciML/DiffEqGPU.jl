...
function SciMLBase.__solve(
    ...
    cpu_trajectories =
        (
            (ensemblealg isa EnsembleGPUArray || ensemblealg isa EnsembleGPUAutonomous) &&
...
diffeqgpunorm(u::AbstractArray{<:ForwardDiff.Dual}, t) =
...
function batch_solve(
    ...
    u0 = reduce(hcat, Array(probs[i].u0) for i = 1:length(I))
    p = reduce(
        hcat,
        probs[i].p isa SciMLBase.NullParameters ? probs[i].p : Array(probs[i].p) for
        i = 1:length(I)
    )

    if ensemblealg isa EnsembleGPUAutonomous
        ps = CuArray([SVector{length(probs[i].p)}(probs[i].p) for i = 1:length(I)])
        if typeof(alg) <: GPUTsit5
            #Adaptive version only works with saveat
            if adaptive
                ts, us =
                    vectorized_asolve(ensembleprob.prob, ps, GPUSimpleATsit5(); kwargs...)
            else
                ts, us =
                    vectorized_solve(ensembleprob.prob, ps, GPUSimpleTsit5(); kwargs...)
            end
            solus = Array(us)
            [
...
function batch_solve_up(ensembleprob, probs, alg, ensemblealg, I, u0, p; kwargs...)
    ...
    if SciMLBase.has_jac(probs[1].f)
        jac_prototype =
            ensemblealg isa EnsembleGPUArray ? cu(zeros(Float32, len, len, length(I))) :
    ...
    us = Array.(sol.u)
    solus = [[@view(us[i][:, j]) for i = 1:length(us)] for j = 1:length(probs)]
    (sol, solus)
end
...
function seed_duals(
    ...
    duals = [ForwardDiff.Dual{T}(x[i, j], seeds[i]) for i = 1:size(x, 1), j = 1:size(x, 2)]
end
...
function ChainRulesCore.rrule(
    ...
    us = Array.(sol.u)
    solus = [
        [ForwardDiff.value.(@view(us[i][:, j])) for i = 1:length(us)] for
        j = 1:length(probs)
    ]
...
function generate_problem(prob::ODEProblem, u0, p, jac_prototype, colorvec)
    ...
    prob = ODEProblem(f_func, u0, prob.tspan, p; prob.kwargs...)
end
...
function generate_problem(prob::SDEProblem, u0, p, jac_prototype, colorvec)
    ...
    prob = SDEProblem(f_func, _g, u0, prob.tspan, p; prob.kwargs...)
end

function generate_callback(callback::DiscreteCallback, I, ensemblealg)
    if ensemblealg isa EnsembleGPUArray
        cur = CuArray([false for i = 1:I])
    else
        cur = [false for i = 1:I]
    end
    ...
    condition = function (u, t, integrator)
        ...
        wait(
            version,
            discrete_condition_kernel(version)(
                _condition,
                cur,
                u,
                t,
    ...
    affect! = function (integrator)
        ...
        wait(
            version,
            discrete_affect!_kernel(version)(
                _affect!,
                cur,
                integrator.u,
                integrator.t,
...
function generate_callback(callback::ContinuousCallback, I, ensemblealg)
    ...
    affect! = function (integrator, event_idx)
        ...
        wait(
            version,
            continuous_affect!_kernel(version)(
                _affect!,
                event_idx,
                integrator.u,
                integrator.t,
...
function LinearSolve.init_cacheval(
    linsol::LinSolveGPUSplitFactorize,
    A,
    b,
    u,
    Pl,
...
function __printjac(A, ii)
    ...
    @cuprintf "%d %d %d\n%d %d %d\n%d %d %d\n" ii[1, 1] ii[1, 2] ii[1, 3] ii[2, 1] ii[2, 2] ii[
        2,
        3,
    ] ii[3, 1] ii[3, 2] ii[3, 3]
    @cuprintf "%2.2f %2.2f %2.2f\n%2.2f %2.2f %2.2f\n%2.2f %2.2f %2.2f" A[ii[1, 1]] A[ii[
        1,
        2,
    ]] A[ii[1, 3]] A[ii[2, 1]] A[ii[2, 2]] A[ii[2, 3]] A[ii[3, 1]] A[ii[3, 2]] A[ii[3, 3]]
end
...
function naivesub!(A::MyU, b::AbstractVector, n)
    ...
    @inbounds for j = n:-1:1
        ...
        for i = j-1:-1:1 # counterintuitively 1:j-1 performs slightly better
            b[i] -= A.data[i, j] * xj
        end
    end
    return nothing
...
function solve_batch(
    ...
    if typeof(prob.prob) <: SciMLBase.AbstractJumpProblem && length(II) != 1
        probs = [deepcopy(prob.prob) for i = 1:Threads.nthreads()]
    else
        probs = prob.prob
    end
...
function tmap(f, args...)
    batch_data =
        Vector{Core.Compiler.return_type(f, Tuple{typeof.(getindex.(args, 1))...})}(
            undef,
            length(args[1]),
        )
    Threads.@threads for i = 1:length(args[1])

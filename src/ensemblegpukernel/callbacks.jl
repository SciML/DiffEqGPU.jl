struct GPUDiscreteCallback{F1, F2, F3, F4, F5} <: SciMLBase.AbstractDiscreteCallback
    ...
    finalize::F4
    save_positions::F5
    function GPUDiscreteCallback(
            condition::F1, affect!::F2,
            initialize::F3, finalize::F4,
            save_positions::F5
        ) where {F1, F2, F3, F4, F5}
...
function GPUDiscreteCallback(
    ...
    return GPUDiscreteCallback(condition, affect!, initialize, finalize, save_positions)
end

function Base.convert(::Type{GPUDiscreteCallback}, x::T) where {T <: DiscreteCallback}
    return GPUDiscreteCallback(
        x.condition, x.affect!, x.initialize, x.finalize,
...
struct GPUContinuousCallback{F1, F2, F3, F4, F5, F6, T, T2, T3, I, R} <:
    ...
    reltol::T2
    repeat_nudge::T3
    function GPUContinuousCallback(
            ...
            F1, F2, F3, F4, F5, F6, T, T2,
            T3, I, R,
        }
        if save_positions != (false, false)
            error("Callback `save_positions` are incompatible with kernel-based GPU ODE solvers due requiring static sizing. Please ensure `save_positions = (false,false)` is set in all callback definitions used with such solvers.")
        end
        return new{F1, F2, F3, F4, F5, F6, T, T2, T3, I, R}(
            condition,
            affect!, affect_neg!,
...
function Base.convert(::Type{GPUContinuousCallback}, x::T) where {T <: ContinuousCallback}
    return GPUContinuousCallback(
        x.condition, x.affect!, x.affect_neg!, x.initialize, x.finalize,
...
function generate_callback(
    ...
    condition = function (u, t, integrator)
        ...
        discrete_condition_kernel(version)(
            _condition, cur, u, t, integrator.p;
            ndrange = size(u, 2),
            workgroupsize = wgs
        )
        return any(cur)
    ...
    affect! = function (integrator)
        ...
        return discrete_affect!_kernel(version)(
            _affect!, cur, integrator.u, integrator.t,
            integrator.p;
